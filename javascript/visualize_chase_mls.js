...
<?php
	...
	$sql .= "WHERE MatchTypeID = 21 AND YEAR(MatchTime) = " . $intThisYear . " ";
	...
	while($row = @mysqli_fetch_array($teams, MYSQLI_ASSOC)) {
		...
		$sql .= "WHERE (HteamID = ".$intTeamID." OR ATeamID = ".$intTeamID.") AND MatchTypeID = 21 AND MatchTime < NOW() AND YEAR(matchtime) = " . $intThisYear . " ";
